...
void gmx::LegacySimulator::do_md()
{
    ...
    while (!bLastStep)
    {
        ...
        else
        {
            ...
            int rankkkkk;
            MPI_Comm_rank(MPI_COMM_WORLD, &rankkkkk);
            std::cout << "step: " << step << ' ' << "rank: "  << rankkkkk << ' ' << "do_force running time: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << std::endl;
        }
